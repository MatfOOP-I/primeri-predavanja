package rs.math.oop.g18.t16a.obradaFilterMapReduceOsobe;

public class LambdaTest01 {

        public static void main(String[] args) {
                System.out.println(System.lineSeparator() + "0.---");
                Person.LIKOVI_CRTANI.stream().forEach(p -> p.printPerson());

                System.out.println(System.lineSeparator() + "1.---");
                Person.LIKOVI_CRTANI.stream().filter(p -> {
                ...
                Person.LIKOVI_CRTANI.stream().filter(x -> (x.gender == Person.Sex.MALE)).map(y -> y.ime.length())
                                .forEach(i -> System.out.print(i + " "));

                System.out.println(System.lineSeparator() + "3.---");
                Person.LIKOVI_CRTANI.stream().filter(p -> p.gender == Person.Sex.MALE).map(p -> p.getAge() - 50)
                                .forEach(x -> System.out.println(x * x));

                System.out.println(System.lineSeparator() + "4.---");
                long sumaGodina = Person.LIKOVI_CRTANI.stream()
                                .filter(p -> p.gender == Person.Sex.MALE && p.emailAddress.indexOf("wb") >= 0)
                ...
                double proizvodGodina2 = Person.LIKOVI_CRTANI.stream()
                                .filter(p -> p.gender == Person.Sex.MALE && p.emailAddress.indexOf("wb") >= 0)
                                .map(p -> p.getAge()).reduce(1L, (x, y) -> x * y);
                System.out.println(proizvodGodina2);

                System.out.println(System.lineSeparator() + "7.---");
                long minGodina = Person.LIKOVI_CRTANI.stream().filter(p -> {
                        return p.gender == Person.Sex.MALE && p.emailAddress.indexOf("disney") >= 0;
                }).map(p -> p.getAge()).reduce(Long.MAX_VALUE, (x, y) -> (x < y) ? x : y);
